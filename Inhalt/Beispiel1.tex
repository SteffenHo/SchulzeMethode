...
Dieses Ergebnis der Wahl wird nun in die \schulze überführt und bewertet.
\newpage
...
In den Graph wurden gerichtete Kanten vom Sieger in die Richtung des Verlierers gezogen und das Kantengewicht des Siegers der Duelle aufgetragen. Nun muss aus der Definition in Abschnitt \ref{itm:def233} (2.3.2) und  (2.3.3) angewendet werden, um die Menge $P$ zu bilden, die die Stärksten Wege beinhaltet.
...
\input{Tabellen/Tabelle_beispiel1_p}

Nun lässt man wieder die Kandidaten gegeneinander antreten dieses mal über die Menge $P$. Wenn es keinen Weg zwischen den Kandidaten gibt, also in der Tabelle ein '---' steht, so wird diese Verbindung als die Stärke 0 betrachtet.
...
\subsection{Ergebnis} 
\label{sec:ergebnis1}
Um den Sieger aus der Relation $\mathcal{O}$ zu finden muss wie in der Definition in Abschnitt \ref{itm:def235} Punkt (2.3.5) beschreiben, sichergestellt werden, das ein Sieger nie durch einen anderen Gegner geschlagen wird.
